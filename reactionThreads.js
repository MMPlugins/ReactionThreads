module.exports = async function ({ bot, config, commands, hooks, threads }) {
    ...
    for (const reaction of reactions) {
        if (typeof reaction.categoryId !== "undefined") {
            ...
            console.info(`[ReactionThreads] Successfully updated category internal data for ${reaction.emoji
                } on https://discord.com/channels/${config.mainServerId}/${reaction.channelId}/${reaction.messageId}`);
        }
    }
    saveReactions();
    ...
    const hasResponse = function (channelId, messageId, emoji) {
        for (const reaction of reactions) {
            if (reaction.channelId == channelId && reaction.messageId == messageId && reaction.emoji == emoji && reaction.response) {
                return reaction;
            }
        }
        return null;
    ...
    const hasDescription = function (channelId, messageId, emoji) {
        for (const reaction of reactions) {
            if (reaction.channelId == channelId && reaction.messageId == messageId && reaction.emoji == emoji && reaction.description) {
                return reaction;
            }
        }
        return null;
    ...
    const addReactionDescCmd = async (message, args) => {
        ...
        reaction = isValidReaction(args.channelId, args.messageId, args.emoji)
        if (reaction == null) {
            message.channel.createMessage(`⚠️ Unable to add reaction description: That reaction doesnt exist on that message!`);
            return;
        }
    ...
    const removeReactionCatCmd = async (message, args) => {
        ...
        reaction = isValidReaction(args.channelId, args.messageId, args.emoji)
        if (reaction == null) {
            message.channel.createMessage(`⚠️ Unable to remove reaction category: That reaction doesnt exist on that message!`);
            return;
        }
        let result = [];
        for (category of reaction.categoryIds) {
        ...
        if (reaction.categoryIds.length === result.length) {
            message.channel.createMessage(`⚠️ Unable to remove reaction category: That category doesnt exist on that reaction!`);
            return;
        }
        delete reaction.categoryIds;
        reaction.categoryIds = result;
        saveReactions();
        message.channel.createMessage("Succesfully removed reaction category from message and de-registered it internally.");
    }
            ...
            if (reaction != null && userHasThread == null) {
                ...
                if (reaction.categoryIds) {
                    ...
                    if (foundCategoryChannel === null) {
                        errorReply = `⚠️ **ReactionThreads:** Failed to move thread into new category. ${reaction.categoryIds.length > 1 ? `Categories` : `Category`} for ${reaction.emoji
                ...
                newThread.postSystemMessage(
                    `:gear: **ReactionThreads:** Thread opened because of reaction ${stringifiedEmoji} to https://discord.com/channels/${message.channel.guild.id
                ...
                newThread.sendSystemMessageToUser(`${reaction.response ? reaction.response : responseMessage
                    }${config["reactionThreads-menu"] == false ? `` : ` (You can change your selection by sending \`` + config.prefix + `menu\`)`}`,
                    { postToThreadChannel }).catch(e => {
                        newThread.postSystemMessage("⚠️ Could not open DMs with the user. They may have blocked the bot or set their privacy settings higher.")
                    });
    ...
    const sendNewMenu = async function (beforeNewThreadData) {
        ...
        if ((beforeNewThreadData.message.content === `${config.prefix}menu`) &&
            (config["reactionThreads-menu"] != false)) return;
        try {
    ...
    commands.addGlobalCommand(
        "menu",
        [{ name: "anyText", type: "string", required: false, catchAll: true }],
        menuCmd,
    )
